...
def trace_nn(
    ...
    Estimates = {vname_dict[name]: np.full(n, np.nan) for name in desired_variables}
    for i in tqdm(range(p), disable=(not verbose_tf), desc="Pref Props"):
        ...
        if (  # there are definitely cleaner ways to do this
            not all([have_vars[k] for k in np.argwhere(need_vars)[:, 0]]) and verbose_tf
        ):
            warnings.warn(
        ...
        if need_vars[1]:
            if eos == "seawater":
                ...
                m[:, 1] = pt0_from_t(
                    SA_from_SP(m[:, 0], p_from_z(C[:, 2], C[:, 1]), C[:, 0], C[:, 1]),
                    m[:, 1],
                    p_from_z(C[:, 2], C[:, 1]),
                )
        ...
        if Est[SoAfrInds].size > 0:
            Est[SoAfrInds] = Est[SoAfrInds] * ((27 - C[SoAfrInds, 0]) / 8) + est_other[
                SoAfrInds
            ] * ((C[SoAfrInds, 0] - 19) / 8)
...
def execute_nn(X, VName, Location, Equation, Net, DATADIR, verbose_tf=True):
    ...
    for ts in tqdm(range(TS), disable=(not verbose_tf), leave=False, desc="Locations"):
        if Net == 0:
            Xp1 = mapminmax_apply(X[ts], x1_step1xoffset, x1_step1gain, x1_step1ymin)
            a1 = tansig_apply(tile_n_dot(b1, IW1_1, Xp1.T))
            a2 = tile_n_dot(np.array([[b2]]), LW2_1, a1)
            Y[ts] = mapminmax_reverse(a2, y1_step1xoffset, y1_step1ygain, y1_step1ymin)[
                0
            ][0]
        else:
            Xp1 = mapminmax_apply(X[ts], x1_step1xoffset, x1_step1gain, x1_step1ymin)
            ...
            Y[ts] = mapminmax_reverse(a3, y1_step1xoffset, y1_step1ygain, y1_step1ymin)[
                0
            ][0]
...
def execute_nn_combined(X, VName, Equation, Net, DATADIR, verbose_tf=True):
    ...
    X = np.array(X)
    dillAtl = dill[
        "TRACE_" + VName + "_" + str(Equation) + "_" + "Atl" + "_" + str(Net + 1)
    ]
    dillOther = dill[
        "TRACE_" + VName + "_" + str(Equation) + "_" + "Other" + "_" + str(Net + 1)
    ]
...
def execute_nn_combined_vectorized(
    ...
    if delta_over_gamma is None:  # default 1.3
        ...
        with open(joinpath(DATADIR, "nn_params_variable_igttd.pkl"), "rb") as f:
            dill = pickle.load(f)
        X = np.array(X)
    ...
    if Net == 0:
        # Atl
        Xp1 = mapminmax_apply(X, x1_step1xoffsetAtl, x1_step1gainAtl, x1_step1yminAtl)
        a1 = tansig_apply(tile_n_dot(b1Atl, IW1_1Atl, Xp1.T))
        a2 = tile_n_dot(np.array([[b2Atl]]), LW2_1Atl, a1)
